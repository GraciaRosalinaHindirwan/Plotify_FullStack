...
<img src="{{ asset($agent['profile']) }}" alt="building" class="w-full rounded-[16px] mb-[16px]">

<form action="" method="POST">
    <h1 class="text-[var(--color-text)] mb-[16px] font-[var(--fw-bold)] text-[18px] text-center">{{ $agent['agentName'] }}</h1>

    <div class="w-full flex text-[var(--color-text)] gap-[8px] items-center pb-[8px]">
        <img src="/img/call.png" alt="call icon">
        <span class="text-[var(--color-text)]">{{ $agent['phone'] }}</span>
    </div>
    <div class="w-full flex text-[var(--color-text)] gap-[8px] mb-[16px] items-center">
        <img src="/img/location.png" alt="location icon">
        <span class="text-[var(--color-text)]">{{ $agent['location'] }}</span>
    </div>
